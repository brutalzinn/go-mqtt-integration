...
func main() {
	...
	if _, err := os.Stat(config.Assets); os.IsNotExist(err) {
		os.Mkdir(config.Assets, os.ModePerm)
		logrus.Info("Dir for assets is created at %s", config.Assets)
	}
	...
	app.Get("/content-url", func(c *fiber.Ctx) error {
		files, err := filepath.Glob(filepath.Join(config.Assets, "*"))
		if err != nil {
		...
		for i, file := range files {
			urls[i] = filepath.Join(config.Assets, filepath.Base(file))
		}
		return c.JSON(urls)
	})
	app.Get("/ws", websocket.New(wshelper.WebSocketHandler))
	app.Static("/assets", filepath.Join(config.Assets))
